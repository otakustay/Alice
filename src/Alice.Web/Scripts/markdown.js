...
(function(exports) {
    ...
    function array_indexOf(array, obj) {
        if (array.indexOf !== undefined) return array.indexOf(obj);
    ...
    function splice_array(dest, position, del, ins) {
        return dest.slice(0, position)
            .concat(ins)
            .concat(dest.slice(position + del));
    }
    ...
    Markdown.prototype.GetListItems = function(input, offset) {
        ...
        if (block.blockType != BlockType_ul && block.blockType != BlockType_ol) return null;
    ...
    Markdown.prototype.OnQualifyUrl = function(url) {
        ...
        if (starts_with(url, "/")) {
            ...
            if (!ends_with(this.UrlBaseLocation, "/")) return this.UrlBaseLocation + "/" + url;
            else return this.UrlBaseLocation + url;
        }
    }
    ...
    Markdown.prototype.OnPrepareLink = function(tag) {
        ...
        // New window?
        if ((this.NewWindowForExternalLinks && IsUrlFullyQualified(url)) || (this.NewWindowForLocalLinks && !IsUrlFullyQualified(url))) {
    ...
    p.ProcessBlocks = function(str) {
        ...
        // Process blocks
        return new BlockProcessor(this, this.MarkdownInHtml)
            .Process(str);
    }
    ...
    p.MakeUniqueHeaderID = function(strHeaderText, startOffset, length) {
        if (!this.AutoHeadingIDs) return null;
    ...
    function UnescapeString(str, ExtraMode) {
        ...
        while (bspos >= 0) {
            if (is_escapable(str.charAt(bspos + 1), ExtraMode)) {
                if (bspos > piece) b.Append(str.substr(piece, bspos - piece));
        ...
        if (piece < str.length) b.Append(str.substr(piece, str.length - piece));
    ...
    function IsEmailAddress(str) {
        ...
        if (posLastDot < posAt) return false;
    ...
    function IsWebAddress(str) {
        ...
        if (str.substr(0, 8) == "https://") return true;
        if (str.substr(0, 6) == "ftp://") return true;
        if (str.substr(0, 7) == "file://") return true;
    ...
    function IsValidHtmlID(str) {
        ...
        if (!is_alpha(str.charAt(0))) return false;

        // Check the rest
        for (var i = 0; i < str.length; i++) {
            var ch = str.charAt(i);
            if (is_alphadigit(ch) || ch == '_' || ch == '-' || ch == ':' || ch == '.') continue;
    ...
    function StripHtmlID(str, start, end) {
        ...
        // Skip closing '{'
        if (pos < start || str.charAt(pos) != '}') return null;
        ...
        // Check for the #
        if (pos < start || str.charAt(pos + 1) != '#') return null;

        // Extract and check the ID
        var startId = pos + 2;
        var strID = str.substr(startId, endId - startId);
        if (!IsValidHtmlID(strID)) return null;
    ...
    p.Append = function(value) {
        if (value) this.m_content.push(value);
    }
    p.Clear = function() {
    ...
    p.HtmlEncode = function(str, startOffset, length) {
        ...
        for (i = startOffset; i < end; i++) {
            switch (str.charAt(i)) {
            case '&':
                if (i > piece) this.Append(str.substr(piece, i - piece));
                this.Append("&");
                piece = i + 1;
                break;
    ...
    p.eol = function() {
        if (this.m_position >= this.end) return true;
    ...
    p.current = function() {
        if (this.m_position >= this.end) return "\0";
    ...
    p.SkipToEol = function() {
        ...
        if (this.m_position < 0) this.m_position = this.end;
    }

    p.SkipEol = function() {
        var save = this.m_position;
        if (this.buf.charAt(this.m_position) == '\r') this.m_position++;
    ...
    p.CharAtOffset = function(offset) {
        if (this.m_position + offset >= this.end) return "\0";
    ...
    p.FindOneOf = function(charset) {
        ...
        for (var ch in charset) {
            ...
            // Search again?
            if (charset_info.m_searched_from == -1 || this.m_position < charset_info.m_searched_from || (this.m_position >= charset_info.m_found_at && charset_info.m_found_at != -1)) {
    ...
    p.Extract = function() {
        if (this.mark >= this.m_position) return "";
        else return this.buf.substr(this.mark, this.m_position - this.mark);
    }
    p.SkipIdentifier = function() {
        ...
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '_') {
            ...
            while (true) {
                ...
                if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '_' || (ch >= '0' && ch <= '9')) this.m_position++;
                else return true;
            }
        }
        return false;
    ...
    p.SkipFootnoteID = function() {
        ...
        while (true) {
            var ch = this.current();
            if (is_alphadigit(ch) || ch == '-' || ch == '_' || ch == ':' || ch == '.' || ch == ' ') this.SkipForward(1);
            else break;
        }
    ...
    p.SkipHtmlEntity = function() {
        if (this.buf.charAt(this.m_position) != '&') return false;
    ...
    p.SkipEscapableChar = function(ExtraMode) {
        if (this.buf.charAt(this.m_position) == '\\' && is_escapable(this.buf.charAt(this.m_position + 1), ExtraMode)) {
            ...
            if (this.m_position < this.end) this.m_position++;
            return false;
        }
    }
    ...
    p.IsSafe = function() {
        ...
        if (!allowed_tags[name_lower]) return false;
        ...
        for (var i in this.attributes) {
            if (!allowed[i.toLowerCase()]) return false;
        }

        // Check href attribute is ok
        if (this.attributes["href"]) {
            if (!IsSafeUrl(this.attributes["href"])) return false;
        }

        if (this.attributes["src"]) {
            if (!IsSafeUrl(this.attributes["src"])) return false;
        }
    ...
    function IsSafeUrl(url) {
        url = url.toLowerCase();
        return (url.substr(0, 7) == "http://" || url.substr(0, 8) == "https://" || url.substr(0, 6) == "ftp://");
    }
    ...
    function ParseHtmlTagHelper(p) {
        ...
        // Get the tag name
        p.Mark();
        if (!p.SkipIdentifier()) return null;
...
*/
    ...
    function ParseLinkDefinition(p, ExtraMode) {
        ...
        if (l == null) p.m_position = savepos;
        return l;
    }
    ...
    p.MakeID = function(str, start, len) {
        ...
        while (!p.eof()) {
            var ch = p.current();
            if (is_alphadigit(ch) || ch == '_' || ch == '-' || ch == '.') sb.Append(ch.toLowerCase());
    ...
    p.Tokenize = function(str, start, len) {
        ...
        while (!p.eof()) {
            ...
            case '_':
                ...
                token = this.CreateEmphasisMark(emphasis_marks && emphasis_marks[emphasis_marks.length - 1]);
            ...
            case '!':
                ...
                if (token == null) p.m_position = linkpos;
                break;
            ...
            case '<':
                ...
                if (tag != null) {
                    ...
                    if (token == null) p.m_position = save;
                }
                break;
    ...
    p.CreateEmphasisMark = function(previous) {
        var p = this.m_Scanner;
        ...
        if (bSpaceAfter || (previous && previous.type === TokenType_opening_mark)) {
            return this.CreateToken(TokenType_closing_mark, savepos, p.m_position - savepos);
        }

        if (this.m_Markdown.ExtraMode && ch == '_') return null;
    ...
    p.ResolveEmphasisMarks = function(tokens, marks) {
        ...
        while (bContinue) {
            ...
            for (var i = 0; i < marks.length; i++) {
                ...
                if (opening_mark.type != TokenType_opening_mark && opening_mark.type != TokenType_internal_mark) continue;
                ...
                for (var j = i + 1; j < marks.length; j++) {
                    ...
                    if (closing_mark.type != TokenType_closing_mark && closing_mark.type != TokenType_internal_mark) break;

                    // Ignore if different type (ie: `*` vs `_`)
                    if (input.charAt(opening_mark.startOffset) != input.charAt(closing_mark.startOffset)) continue;
    ...
    p.ProcessAutoLink = function() {
        if (this.m_DisableLinks) return null;
    ...
    p.ProcessLinkOrImageOrFootnote = function() {
        ...
        if (this.m_DisableLinks && token_type == TokenType_link) return null;
    ...
    p.ProcessCodeSpan = function() {
        ...
        // End?
        if (p.eof()) return this.CreateToken(TokenType_Text, start, p.m_position - start);

        var startofcode = p.m_position;

        // Find closing ticks
        if (!p.Find(p.buf.substr(start, tickcount))) return this.CreateToken(TokenType_Text, start, p.m_position - start);
    ...
    p.CreateToken = function(type, startOffset, length) {
        if (this.m_SpareTokens.length != 0) {
            ...
            t.data = null;
            return t;
        } else return new Token(type, startOffset, length);
    }
    ...
    p.get_Content = function() {
        if (this.buf == null) return null;
        if (this.contentStart == -1) return this.buf;
    ...
    p.Render = function(m, b) {
        ...
        case BlockType_h6:
            if (m.ExtraMode && !m.SafeMode) {
                b.Append("<h" + (this.blockType - BlockType_h1 + 1)
                    .toString());
        ...
        case BlockType_dd:
            ...
            if (this.children != null) {
                b.Append("\n");
                this.RenderChildren(m, b);
            } else m.m_SpanFormatter.Format(b, this.buf, this.contentStart, this.contentLen);
            b.Append("</dd>\n");
            break;

        case BlockType_dt:
            if (this.children == null) {
                var lines = this.get_Content()
                    .split("\n");
    ...
    p.StartTable = function(p, spec, lines) {
        ...
        if (lines.length == 1) {
            ...
            if (spec.m_Headers == null) return false;
            p.m_position = savepos;
            lines.length = 0;
        }
    ...
    p.EvaluateLineInternal = function(p, b) {
        // Empty line?
        if (p.eol()) return BlockType_Blank;
        ...
        if (this.m_Markdown.ExtraMode) {
            ...
            // Fenced code blocks?
            if (ch == '~') {
                if (this.ProcessFencedCodeBlock(p, b)) return b.blockType;
        ...
        while (!p.eol()) {
            if (p.current() == ' ') {
                if (tabPos < 0) leadingSpaces++;
            } else if (p.current() == '\t') {
                if (tabPos < 0) tabPos = p.m_position;
            } else {
        ...
        // Html block?
        if (ch == '<') {
            if (this.ScanHtml(p, b)) return b.blockType;
    ...
    p.GetMarkdownMode = function(tag) {
        ...
        if (md == undefined) {
            if (this.m_bMarkdownInHtml) return MarkdownInHtmlMode_Deep;
            else return MarkdownInHtmlMode_NA;
        }
        ...
        // Parse mode
        if (md == "1") return (tag.get_Flags() & HtmlTagFlags_ContentAsSpan) != 0 ? MarkdownInHtmlMode_Span : MarkdownInHtmlMode_Block;

        if (md == "block") return MarkdownInHtmlMode_Block;

        if (md == "deep") return MarkdownInHtmlMode_Deep;

        if (md == "span") return MarkdownInHtmlMode_Span;
    ...
    p.ProcessMarkdownEnabledHtml = function(p, b, openingTag, mode) {
        ...
        while (!p.eof()) {
            ...
            if (this.m_Markdown.SafeMode && mode == MarkdownInHtmlMode_Off && !bHasUnsafeContent) {
                if (!tag.IsSafe()) bHasUnsafeContent = true;
            }
    ...
    p.ScanHtml = function(p, b) {
        ...
        // Safe mode?
        var bHasUnsafeContent = false;
        if (this.m_Markdown.SafeMode && !openingTag.IsSafe()) bHasUnsafeContent = true;

        var flags = openingTag.get_Flags();

        // Is it a block level tag?
        if ((flags & HtmlTagFlags_Block) == 0) return false;
        ...
        while (!p.eof()) {
            ...
            // Safe mode checks
            if (this.m_Markdown.SafeMode && !tag.IsSafe()) bHasUnsafeContent = true;
    ...
    p.BuildList = function(lines) {
        ...
        for (var i = 1; i < lines.length; i++) {
            ...
            if ((lines[i].blockType == BlockType_p) && (lines[i - 1].blockType == BlockType_p || lines[i - 1].blockType == BlockType_ul_li || lines[i - 1].blockType == BlockType_ol_li)) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (start_of_li == end_of_li) {
                ...
                List.children.push(this.CreateBlock()
                    .CopyFrom(lines[i]));
            } else {
    ...
    p.BuildDefinition = function(lines) {
        ...
        for (var i = 1; i < lines.length; i++) {
            // Join plain paragraphs
            if ((lines[i].blockType == BlockType_p) && (lines[i - 1].blockType == BlockType_p || lines[i - 1].blockType == BlockType_dd)) {
                lines[i - 1].set_contentEnd(lines[i].get_contentEnd());
                this.FreeBlock(lines[i]);
                lines.splice(i, 1);
                i--;
    ...
    p.BuildFootnote = function(lines) {
        ...
        for (var i = 1; i < lines.length; i++) {
            // Join plain paragraphs
            if ((lines[i].blockType == BlockType_p) && (lines[i - 1].blockType == BlockType_p || lines[i - 1].blockType == BlockType_footnote)) {
                lines[i - 1].set_contentEnd(lines[i].get_contentEnd());
                this.FreeBlock(lines[i]);
                lines.splice(i, 1);
                i--;
    ...
    p.ProcessFencedCodeBlock = function(p, b) {
        ...
        if (strFence.length < 3) return false;
        ...
        // Find the end fence
        if (!p.Find(strFence)) return false;

        // Character before must be a eol char
        if (!is_lineend(p.CharAtOffset(-1))) return false;
    ...
    p.ParseRow = function(p) {
        ...
        if (p.eol()) return null; // Blank line ends the table
    ...
    function TableSpec_Parse(p) {
        ...
        if (p.current() != '|' && p.current() != ':' && p.current() != '-') return null;
        ...
        while (true) {
            ...
            if (AlignLeft && AlignRight) col = ColumnAlignment_Center;
            else if (AlignLeft) col = ColumnAlignment_Left;
            else if (AlignRight) col = ColumnAlignment_Right;
            ...
            if (spec == null) spec = new TableSpec();
